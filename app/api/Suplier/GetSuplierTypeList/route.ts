...
export async function GET(_request: NextRequest) {
  try {
    const response = await fetch(supplierApiUpstreamUrl('Suplier/GetSuplierTypeList'), {
      method: 'GET',
      headers: {
        ...SUPPLIER_AUTH_HEADER,
    ...
    return forwardSupplierJsonResponse(response, 'GetSuplierTypeList');
  } catch (error) {
    console.warn('[supplier-api] GetSuplierTypeList fetch failed:', error);
