...
export async function GET(_request: NextRequest) {
  try {
    const response = await fetch(supplierApiUpstreamUrl('Suplier/GetSuplierStatusList'), {
      method: 'GET',
      headers: {
        ...SUPPLIER_AUTH_HEADER,
    ...
    return forwardSupplierJsonResponse(response, 'GetSuplierStatusList');
  } catch (error) {
    console.warn('[supplier-api] GetSuplierStatusList fetch failed:', error);
